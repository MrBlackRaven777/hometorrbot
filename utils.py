...
def create_markup_new(button_list, id, pg_num=1):
    n_rows = [0, 0, 0]
    blen = len(button_list)
    keyboard = []
    boobs = 9 #not my fetish, just max number of Buttons On One Board Setting =) 
    keyboard = [[button_list[x+y] for x in range(2)] for y in range(2)]
    print(keyboard)
#    for i in range(boobs):
#        keyboard[k]=keyboard[k].append(button_list[i])
